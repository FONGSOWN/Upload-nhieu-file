...
class UploadApp:
    ...
    def _on_closing(self):
        """Hàm xử lý khi người dùng nhấn nút X để đóng ứng dụng"""
        # Đếm số lượng file đang trong trạng thái UPLOADING
        uploading_count = sum(1 for r in self.rows.values() if r.status == STATUS_UPLOADING)
        
        if uploading_count > 0:
            # Hiển thị hộp thoại cảnh báo người dùng
            if messagebox.askokcancel("Xác nhận thoát", f"Đang có {uploading_count} file đang upload. Bạn có chắc muốn thoát?"):
                self.root.destroy()
        else:
            # Nếu không có file nào đang upload thì đóng ứng dụng bình thường
            self.root.destroy()
