...
class FileRow:
    ...
    def __init__(self, parent, filepath, index, style):
        ...
        # Progress bar
        bar_style_name = (
            f"Row{index}.Horizontal.TProgressbar"
        )

        self.style.configure(
            bar_style_name,
            troughcolor="#e5e7eb",
            background=STATUS_STYLE[
                STATUS_WAIT
            ]["bar"],
            bordercolor="#e5e7eb",
            lightcolor=STATUS_STYLE[
                STATUS_WAIT
            ]["bar"],
            darkcolor=STATUS_STYLE[
                STATUS_WAIT
            ]["bar"],
            thickness=10
        )
...
class UploadApp:
    ...
    def __init__(self, root):
        ...
        self.root.title(
            "UDM_10 - Upload nhieu file len Server"
        )
    ...
    def _build_ui(self):
        ...
        server_pill = tk.Label(
            top,
            text=f"\U0001F5A5  {config.HOST}:{config.PORT}",
            bg="#eef2ff",
            fg=COL_PRIMARY_DARK,
        ...
        if DND_AVAILABLE:
            ...
            drop_text = (
                "\u2B07  Keo & tha file vao day de upload"
            )

            drop_sub = (
                "hoac bam nut 'Chon file...' phia tren"
            )
            ...
            drop_text = (
                "\u26A0  Chua cai tkinterdnd2 nen khong keo-tha duoc"
            )

            drop_sub = (
                "chay: pip install tkinterdnd2  —  "
                "hoac dung nut 'Chon file...'"
            )

        drop_border_frame = tk.Frame(
            drop_wrap,
            bg=drop_border
        )

        drop_border_frame.pack(
            fill='x',
            pady=(0, 10)
        )
